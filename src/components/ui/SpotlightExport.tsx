...
interface SpotlightExportProps {
  deputy: Deputy;
...
export function SpotlightExport({ deputy, spotlightTitle }: SpotlightExportProps) {
  ...
  const generateFilename = (ext: string) => {
    ...
    return `ceap-spotlight-${slug}-${date}.${ext}`;
  };
  ...
  const exportAsCSV = () => {
    ...
    try {
      ...
      // Header
      rows.push(['CEAP Spotlight Report']);
  ...
  const exportAsTXT = () => {
    ...
    try {
      ...
      lines.push('CEAP SPOTLIGHT REPORT');
      lines.push('═'.repeat(60));
      lines.push('');
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-bg-card border border-border rounded-lg shadow-xl z-50 overflow-hidden">
            <div className="p-3 border-b border-border">
              <p className="text-sm font-medium text-text-primary">Exportar Spotlight</p>
